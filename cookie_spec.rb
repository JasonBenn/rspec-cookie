...
describe Cookie do
  ...
  describe "#bake!" do
    it "requires an integer time argument" do
      lambda { cookie.bake!('asdf') }.should raise_error(TypeError)
    end

    it "returns the cookie object" do
      expect(cookie.bake!(10)).to be_an_instance_of(Cookie)
    end
  ...
  describe "#status" do
    it "returns the cookie's current status" do
      cookie.stub(time_baked: 5)
      expect(cookie.status).to be_an_instance_of Symbol
    end

    context "when unbaked" do
      it "is `:doughy`" do
        cookie.stub(time_baked: 0)
